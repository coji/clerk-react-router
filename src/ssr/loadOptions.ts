...
export const loadOptions = (
  ...
  overrides: RootAuthLoaderOptions = {},
) => {
  ...
  const secretKey =
    overrides.secretKey || getEnvVariable('CLERK_SECRET_KEY', context) || ''
  const publishableKey =
    overrides.publishableKey ||
    getEnvVariable('CLERK_PUBLISHABLE_KEY', context) ||
    ''
  const jwtKey = overrides.jwtKey || getEnvVariable('CLERK_JWT_KEY', context)
  const apiUrl =
    getEnvVariable('CLERK_API_URL', context) ||
    apiUrlFromPublishableKey(publishableKey)
  const domain =
    handleValueOrFn(overrides.domain, new URL(request.url)) ||
    getEnvVariable('CLERK_DOMAIN', context) ||
    ''
  const isSatellite =
  ...
  const signUpUrl =
    overrides.signUpUrl || getEnvVariable('CLERK_SIGN_UP_URL', context) || ''
  ...
  //   overrides.afterSignInUrl ||
  //   getEnvVariable('CLERK_AFTER_SIGN_IN_URL', context) ||
  //   '';
  // const afterSignUpUrl =
  //   overrides.afterSignUpUrl ||
  //   getEnvVariable('CLERK_AFTER_SIGN_UP_URL', context) ||
  //   '';
